...
function Sparkline({ data = [], color = P.orange, prefix = "" }) {
  ...
  const lx = xs[xs.length - 1],
    ly = ys[ys.length - 1];
  const trend = real
    ? ((raw[raw.length - 1] - raw[0]) / (raw[0] || 1)) * 100
    : 0;
  const up = trend >= 0;
    ...
    >
      ...
      <div
        style={{
          ...
          height: 1,
          background: isDark
            ? `linear-gradient(90deg, ${bdr(isDark)}, transparent)`
            : `linear-gradient(90deg, rgba(100,50,10,0.20), transparent)`,
        }}
      />
    </div>
  );
...
function Widget({
  ...
  useEffect(() => {
    gsap.fromTo(
      ref.current,
      { opacity: 0, y: 16 },
      { opacity: 1, y: 0, duration: 0.48, delay, ease: "power3.out" },
    );
  }, []);
      ...
      <CometCard depth={8} style={{ ...cardBase(isDark), overflow: "hidden" }}>
        ...
        <div
          ...
          style={{
            ...
            padding: "13px 15px",
            cursor: "pointer",
            borderBottom: `1px solid ${div(isDark)}`,
          }}
        >
            ...
            {badge != null && (
              <span
                style={{
                  ...
                  padding: "2px 8px",
                  borderRadius: 99,
                  background: `${badgeColor || color}16`,
                  color: badgeColor || color,
                  fontFamily: "'DM Sans', sans-serif",
                  border: `1px solid ${badgeColor || color}22`,
                  letterSpacing: "0.3px",
                }}
              >
                {badge}
...
function TablesWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api
      .get("/table-session/active")
      .then((r) => setSess(r.data?.sessions ?? r.sessions ?? []))
      .catch(() => {});
  }, []);
  const zClr = { Indoor: P.blue, Outdoor: P.green, Terrace: P.orange };
  const byZone = sess.reduce((a, s) => {
  ...
  return (
    <Widget
      title="Tables"
      sub={`${sess.length} active sessions`}
      icon={Map}
      color={P.green}
      badge={sess.length || null}
      badgeColor={P.green}
      onClick={() => go("tables")}
      delay={0.08}
      isDark={isDark}
            ...
            {Object.entries(byZone).map(([z, c]) => (
              <span
                ...
                style={{
                  ...
                  padding: "2px 9px",
                  borderRadius: 99,
                  background: `${zClr[z] || P.green}13`,
                  color: zClr[z] || P.green,
                  fontFamily: "'DM Sans', sans-serif",
                  border: `1px solid ${zClr[z] || P.green}20`,
                }}
              >
                {z} · {c}
              </span>
...
function StaffWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api
      .get("/staff")
      .then((r) => setStaff((r.data?.staff ?? r.staff ?? []).slice(0, 5)))
      .catch(() => {});
  }, []);
  const rClr = {
    waiter: P.orange,
              ...
              >
                ...
                <div
                  style={{
                    ...
                    height: 7,
                    borderRadius: "50%",
                    background: s.isActive ? P.green : tm(isDark),
                    boxShadow: s.isActive ? `0 0 8px ${P.green}80` : "none",
                    flexShrink: 0,
                  }}
                />
              </div>
...
function InventoryWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api
      ...
      .then((r) => {
        ...
        setLow(bad.slice(0, 4));
        setAllOk(bad.length === 0);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <Widget
      title="Inventory"
      sub={allOk ? "All items stocked" : `${low.length} items running low`}
      icon={Package}
      color="#F97316"
      badge={low.length > 0 ? `${low.length} low` : null}
      badgeColor={P.rose}
      ...
      {allOk ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {low.map((item) => {
            const pct = Math.min(
              100,
              (item.quantity / item.lowThreshold) * 100,
            );
...
function LoyaltyWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api
      ...
      .then((r) => {
        ...
        setTop(lb.slice(0, 3));
        setTiers({
          gold: lb.filter((x) => x.tier === "gold").length,
          silver: lb.filter((x) => x.tier === "silver").length,
          bronze: lb.filter((x) => x.tier === "bronze").length,
        });
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <Widget
      ...
      icon={Star}
      color={P.amber}
      badge={tiers.gold > 0 ? `${tiers.gold} gold` : null}
      badgeColor={P.amber}
      onClick={() => go("loyalty")}
      delay={0.16}
      isDark={isDark}
      ...
      {top.map((m, i) => (
        <div
          ...
          style={{
            ...
            gap: 9,
            padding: "6px 0",
            borderTop: `1px solid ${div(isDark)}`,
          }}
        >
          ...
          >
            {m.points?.toLocaleString("en-IN")} pts
          </span>
        </div>
      ))}
    </Widget>
    ...
    <div ref={ref} style={{ opacity: 0 }}>
      <CometCard depth={8}>
        <div
          onClick={onClick}
          style={{
            background: `linear-gradient(140deg, #FF5500 0%, #CC3300 55%, #991F00 100%)`,
            borderRadius: 18,
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            >
              Rs {loading ? "—" : (daily?.revenue ?? 0).toLocaleString("en-IN")}
            </p>
            <p
              style={{
                fontSize: 12,
            ...
            >
              {daily?.orders ?? 0} orders · avg Rs {daily?.avgOrderValue ?? 0}
            </p>
            {/* Progress bar */}
            <div
              style={{
            ...
            >
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${pct}%` }}
                transition={{ duration: 1.4, ease: "expo.out", delay: 0.5 }}
                style={{
                  height: "100%",
                  borderRadius: 99,
            ...
            >
              {daily?.orders ?? 0} / 50 daily target
            </p>
          </div>
        </div>
      </CometCard>
...
function OrdersList({ orders, loading, onClick, isDark }) {
  ...
  return (
    <CometCard depth={5} style={{ ...cardBase(isDark), overflow: "hidden" }}>
      ...
      <div
        style={{ maxHeight: 280, overflowY: "auto", scrollbarWidth: "none" }}
      >
        {loading ? (
          [1, 2, 3].map((i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 9,
                background: div(isDark),
                animation: "kcShimmer 1.5s ease-in-out infinite",
              }}
            />
          ))
        ) : orders.length === 0 ? (
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "9px 15px",
                  borderBottom: `1px solid ${div(isDark)}`,
                  animation: `kcFadeIn 0.3s ease ${i * 0.05}s both`,
                }}
              >
                <div
                  style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    Table {o.tableNumber ?? o.table?.tableNumber ?? "?"}
                  </p>
                  <p
                    style={{
                      fontSize: 10,
                ...
                >
                  Rs {(o.total ?? o.totalAmount ?? 0).toLocaleString("en-IN")}
                </span>
              </div>
            );
          })
    ...
    >
      {btns.map(({ label, Icon, color, tab }, i) => (
        <CometCard
          ...
          style={{
            ...cardBase(isDark),
            animation: `kcFadeIn 0.35s ease ${i * 0.07}s both`,
            overflow: "hidden",
          }}
        >
          <div
...
function RightSidebar({ daily, orders, loading, isDark, go }) {
  ...
  useEffect(() => {
    gsap.fromTo(
      ref.current,
      { opacity: 0, x: 20 },
      { opacity: 1, x: 0, duration: 0.55, delay: 0.2, ease: "expo.out" },
    );
  }, []);
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 9 }}>
        {[
          ...
          {
            label: "Avg Response",
            val: `${daily?.avgResponseMinutes ?? 0}m`,
            Icon: Clock,
            color: P.blue,
            raw: true,
          },
...
function CoffeeCup({ size = 48 }) {
  ...
  useEffect(() => {
    ...
    const t1 = mkSteam(s1.current, 0, -2),
      t2 = mkSteam(s2.current, 0.6, 2),
      t3 = mkSteam(s3.current, 1.2, -1);
    gsap.to(liq.current, {
      opacity: 0.45,
      duration: 2,
      repeat: -1,
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id="cupLiq" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="#7A3A10" />
          <stop offset="50%" stopColor="#A05020" />
          <stop offset="100%" stopColor="#6B2E0C" />
        </linearGradient>
      </defs>
      <path
        ref={s1}
        d="M20 18 C20 16 23 14 23 12 C23 10 20 8 20 6"
        stroke={P.orange}
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
      ...
      <path
        ref={s2}
        d="M28 17 C28 15 31 13 31 11 C31 9 28 7 28 5"
        stroke="#FFB060"
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
      ...
      <path
        ref={s3}
        d="M36 18 C36 16 33 14 33 12 C33 10 36 8 36 6"
        stroke={P.orangeHi}
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
  ...
  useEffect(() => {
    const anims = [
      gsap
        ...
        .to(b0.current, { x: 55, y: -35, duration: 20, ease: "sine.inOut" }),
      gsap
        .timeline({ repeat: -1, yoyo: true, delay: 7 })
        .to(b1.current, { x: -45, y: 45, duration: 24, ease: "sine.inOut" }),
      gsap
        .timeline({ repeat: -1, yoyo: true, delay: 13 })
        .to(b2.current, { x: 35, y: -40, duration: 17, ease: "sine.inOut" }),
    ];
        ...
        <>
          <div
            ...
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle, rgba(255,75,0,0.06) 0%, transparent 65%)",
              filter: "blur(80px)",
            }}
          />
          <div
            ...
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle, rgba(200,50,0,0.05) 0%, transparent 65%)",
              filter: "blur(90px)",
            }}
          />
          <div
            ...
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle, rgba(255,120,0,0.03) 0%, transparent 65%)",
              filter: "blur(70px)",
            }}
          />
        </>
...
function Overview({ isDark, go }) {
      ...
      .then(([s, d]) => {
        ...
        if (d.status === "fulfilled") {
          ...
          setDaily({
            ...
            avgOrderValue: Math.round(dd.avgOrderValue ?? 0),
            waiterCalls: dd.waiterCalls ?? 0,
            avgResponseMinutes: dd.avgResponseMinutes ?? 0,
          });
        }
      })
  ...
  const avgData =
    revData.length >= 2 && ordData.length >= 2
      ? revData.map((r, i) => (ordData[i] > 0 ? Math.round(r / ordData[i]) : 0))
      : [];
  const callData = daily?.waiterCalls
    ? [0, 0, 0, 0, 0, 0, daily.waiterCalls]
    : [];
        ...
        <div>
          ...
          >
            ...
            <StatCard
              ...
              isDark={isDark}
              delay={0.14}
              sub={`avg Rs ${daily?.avgOrderValue ?? 0}`}
              graphData={ordData}
              onClick={() => go("reports")}
            />
            <StatCard
              label="Waiter Calls"
              value={daily?.waiterCalls ?? 0}
              icon={Bell}
              color={P.blue}
              isDark={isDark}
              delay={0.19}
              sub={
                daily?.avgResponseMinutes
                  ? `${daily.avgResponseMinutes}m avg`
                  : undefined
              }
              graphData={callData}
              onClick={() => go("reports")}
...
function Section({ active, go }) {
  ...
  const mobile = bp === "mobile",
    isOv = active === "overview";
  return (
    <div
      ref={ref}
      style={{
        maxWidth: isOv ? "100%" : 880,
        margin: isOv ? 0 : "0 auto",
        padding: `16px ${mobile ? "12px" : "22px"} ${mobile ? "20px" : "28px"}`,
        height: isOv ? "100%" : "auto",
