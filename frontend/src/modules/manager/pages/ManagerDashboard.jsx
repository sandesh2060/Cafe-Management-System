...
// ═══════════════════════════════════════════════════════════════════════
//  KAUSĪ CHIYĀ  ·  Manager Dashboard
//  ✦ Financial-dashboard aesthetic — pixel-perfect to reference image
//  ✦ Near-black #0D0B09 · Vivid orange #FF5500 · Bold white numerals
//  ✦ DM Sans body · Playfair Display headings · DM Mono figures
//  ✦ CometCard 3D tilt · GSAP enter animations · Sparkline charts
// ═══════════════════════════════════════════════════════════════════════
...
import {
  LayoutDashboard, Users, Map, Package, Star, FileText,
  MessageSquare, TrendingUp, ShoppingBag, IndianRupee,
  Bell, Clock, Activity, ChevronRight, CheckCircle,
  ArrowUpRight, ArrowDownRight, Coffee, Zap,
} from "lucide-react";
import { selectUser } from "@store/slices/authSlice";

// ─── Palette — extracted pixel-perfect from reference image ───────────────────
const P = {
  // Page & surfaces
  pageBg:    "#0D0B09",   // near-black page background
  card:      "#161210",   // card surface — very dark warm brown-black
  cardHi:    "#1D1712",   // card slightly elevated
  cardHov:   "#211913",   // card hover state

  // Brand orange — vivid, saturated (reference uses #FF5500 family)
  orange:    "#FF5500",
  orangeHi:  "#FF7733",
  orangeLo:  "#CC3D00",
  orangeGlo: "rgba(255,85,0,0.18)",
  ...
  // Text hierarchy — matches reference exactly
  textPri:   "#F5F0E8",   // primary — warm white
  textSec:   "#7A6550",   // secondary — muted warm brown
  textMut:   "#3D2E22",   // muted
  textAcc:   "#FF7733",   // orange accent text

  // Borders & dividers
  border:    "rgba(255,85,0,0.09)",
  div:       "rgba(255,255,255,0.05)",

  // Light mode — strong enough contrast to separate cards from page bg
  lBg:       "#E8DDD0",   // warm tan page — clearly darker than cards
  lCard:     "#FAF7F3",   // off-white card — clearly lighter than page
  lTextPri:  "#1A0E04",
  lTextSec:  "#7A5840",
  lBorder:   "rgba(100,50,10,0.20)",   // visible border
  lDiv:      "rgba(0,0,0,0.10)",       // visible divider
}
...
// Card base — light mode shadow gives clear card separation from page bg
...
// ─── CometCard — 3D tilt with orange glare ───────────────────────────────────
...
function AnimNum({ value = 0, prefix = "", suffix = "" }) {
  ...
  useEffect(() => {
    ...
    gsap.to(obj.current, {
      ...
      onUpdate: () => {
        if (el.current)
          el.current.textContent = `${prefix}${Math.round(obj.current.n).toLocaleString("en-IN")}${suffix}`
      },
    })
  }, [value])
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block", overflow: "visible" }}>
        ...
        <path
          ref={lineRef} d={line} fill="none"
          stroke={real ? color : `${color}33`}
          strokeWidth={real ? 2.4 : 1.5}
          strokeLinecap="round" strokeLinejoin="round"
          strokeDasharray={real ? undefined : "4 6"}
          filter={real ? `url(#${uid}glow)` : undefined}
        />
        <g ref={dotRef}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
        <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: `${color}55`, letterSpacing: "0.2px" }}>{fmt(raw[0])}</span>
        {real
          ? <span style={{ fontSize: 10, fontWeight: 700, color: up ? P.green : P.rose, display: "flex", alignItems: "center", gap: 2, fontFamily: "'DM Sans', sans-serif" }}>
              {up ? <ArrowUpRight size={11} strokeWidth={2.5}/> : <ArrowDownRight size={11} strokeWidth={2.5}/>}
              {Math.abs(Math.round(trend))}%
            </span>
          : <span style={{ fontSize: 9, color: `${color}44`, fontFamily: "'DM Sans', sans-serif" }}>no data yet</span>
        }
        <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "'DM Mono', monospace", color, letterSpacing: "0.2px" }}>{fmt(raw[raw.length-1])}</span>
      </div>
    </div>
  )
}
...
function SectionLabel({ children, isDark }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
      <span style={{ fontSize: 9, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1.8px", fontFamily: "'DM Sans', sans-serif", color: ts(isDark), whiteSpace: "nowrap" }}>
        {children}
      </span>
...
function StatCard({ label, value, prefix = "", icon: Icon, color, sub, graphData, graphPrefix, delay = 0, onClick, isDark }) {
  ...
  useEffect(() => {
    gsap.fromTo(ref.current,
      ...
      { opacity: 1, y: 0, scale: 1, duration: 0.55, delay, ease: "back.out(1.8)" }
    )
  }, [])
        ...
        <div onClick={onClick} style={{ padding: "16px 16px 13px", display: "flex", flexDirection: "column", gap: 12, cursor: "pointer" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 30, fontWeight: 900, lineHeight: 1, letterSpacing: "-1.5px", color: tp(isDark), fontFamily: "'DM Sans', sans-serif" }}>
                <AnimNum value={value} prefix={prefix} />
              </div>
              <div style={{ fontSize: 9, fontWeight: 800, marginTop: 4, textTransform: "uppercase", letterSpacing: "1px", color, fontFamily: "'DM Sans', sans-serif" }}>{label}</div>
              {sub && <div style={{ fontSize: 10, marginTop: 2, color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>{sub}</div>}
            </div>
          </div>
      ...
      <CometCard depth={8} style={{ ...cardBase(isDark), overflow: "hidden" }}>
        ...
        <div onClick={onClick} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 15px", cursor: "pointer", borderBottom: `1px solid ${div(isDark)}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: tp(isDark), fontFamily: "'DM Sans', sans-serif", lineHeight: 1.2 }}>{title}</p>
              {sub && <p style={{ fontSize: 10, margin: "2px 0 0", color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>{sub}</p>}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            {badge != null && (
              <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 99, background: `${badgeColor || color}16`, color: badgeColor || color, fontFamily: "'DM Sans', sans-serif", border: `1px solid ${badgeColor || color}22`, letterSpacing: "0.3px" }}>{badge}</span>
            )}
...
function TablesWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api.get("/table-session/active").then(r => setSess(r.data?.sessions ?? r.sessions ?? [])).catch(() => {})
  }, [])
  ...
  return (
    <Widget title="Tables" sub={`${sess.length} active sessions`} icon={Map} color={P.green} badge={sess.length || null} badgeColor={P.green} onClick={() => go("tables")} delay={0.08} isDark={isDark}>
      {sess.length === 0
        ? <p style={{ fontSize: 12, color: ts(isDark), margin: 0, fontFamily: "'DM Sans', sans-serif" }}>No active sessions right now</p>
        : <>
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 11 }}>
            {Object.entries(byZone).map(([z, c]) => (
              <span key={z} style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 99, background: `${zClr[z]||P.green}13`, color: zClr[z]||P.green, fontFamily: "'DM Sans', sans-serif", border: `1px solid ${zClr[z]||P.green}20` }}>{z} · {c}</span>
            ))}
          </div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {sess.slice(0, 9).map((s, i) => {
              ...
              return (
                <div key={i} style={{ width: 34, height: 34, borderRadius: 9, background: `${c}13`, border: `1px solid ${c}25`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: c, fontFamily: "'DM Sans', sans-serif", flexShrink: 0 }}>
                  {s.tableNumber ?? i+1}
                </div>
              )
            })}
            {sess.length > 9 && <div style={{ width: 34, height: 34, borderRadius: 9, background: div(isDark), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>+{sess.length-9}</div>}
          </div>
        </>
      }
    </Widget>
...
function StaffWidget({ isDark, go }) {
  ...
  useEffect(() => {
    api.get("/staff").then(r => setStaff((r.data?.staff ?? r.staff ?? []).slice(0, 5))).catch(() => {})
  }, [])
  ...
  return (
    <Widget title="Staff" sub={`${active} currently on duty`} icon={Users} color={P.blue} badge={active || null} badgeColor={P.green} onClick={() => go("staff")} delay={0.12} isDark={isDark}>
      {staff.length === 0
        ? <p style={{ fontSize: 12, color: ts(isDark), margin: 0, fontFamily: "'DM Sans', sans-serif" }}>Loading…</p>
        : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {staff.map(s => {
            const c = rClr[s.role] || P.orange
            return (
              <div key={s._id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 30, height: 30, borderRadius: 8, background: `linear-gradient(135deg, ${c}CC, ${c}66)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 800, flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}>
                  {(s.name||"?")[0].toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, margin: 0, color: tp(isDark), fontFamily: "'DM Sans', sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</p>
                  <p style={{ fontSize: 10, margin: 0, color: c, fontFamily: "'DM Sans', sans-serif", textTransform: "capitalize" }}>{s.role}</p>
                </div>
    ...
    <Widget title="Inventory" sub={allOk ? "All items stocked" : `${low.length} items running low`} icon={Package} color="#F97316" badge={low.length > 0 ? `${low.length} low` : null} badgeColor={P.rose} onClick={() => go("inventory")} delay={0.14} isDark={isDark}>
      {allOk
        ? <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <p style={{ fontSize: 12, margin: 0, fontWeight: 600, color: P.green, fontFamily: "'DM Sans', sans-serif" }}>All items fully stocked</p>
        </div>
            ...
            return (
              <div key={item._id}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: tp(isDark), fontFamily: "'DM Sans', sans-serif" }}>{item.name}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: c, fontFamily: "'DM Mono', monospace" }}>{item.quantity} {item.unit}</span>
                </div>
  ...
  return (
    <Widget title="Loyalty" sub={`${total} active members`} icon={Star} color={P.amber} badge={tiers.gold > 0 ? `${tiers.gold} gold` : null} badgeColor={P.amber} onClick={() => go("loyalty")} delay={0.16} isDark={isDark}>
      <div style={{ display: "flex", gap: 7, marginBottom: 12 }}>
        {[
          { label: "Gold", count: tiers.gold, color: "#D97706", emoji: "🥇" },
          { label: "Silver", count: tiers.silver, color: "#9CA3AF", emoji: "🥈" },
          { label: "Bronze", count: tiers.bronze, color: "#B45309", emoji: "🥉" },
          ...
          <div key={label} style={{ flex: 1, textAlign: "center", padding: "8px 4px", borderRadius: 10, background: `${color}10`, border: `1px solid ${color}18` }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 900, color, fontFamily: "'DM Sans', sans-serif", letterSpacing: "-0.5px" }}>{count}</div>
            <div style={{ fontSize: 9, color: ts(isDark), fontFamily: "'DM Sans', sans-serif", textTransform: "uppercase", letterSpacing: "0.8px" }}>{label}</div>
          </div>
        ))}
      </div>
      {top.map((m, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 9, padding: "6px 0", borderTop: `1px solid ${div(isDark)}` }}>
          ...
          <p style={{ flex: 1, fontSize: 12, fontWeight: 600, margin: 0, color: tp(isDark), fontFamily: "'DM Sans', sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.userId?.name ?? "Member"}</p>
          <span style={{ fontSize: 11, fontWeight: 700, color: P.amber, fontFamily: "'DM Mono', monospace" }}>{m.points?.toLocaleString("en-IN")} pts</span>
        </div>
      ))}
    </Widget>
  )
...
// ─── Revenue Hero — main hero card (right column on desktop) ──────────────────
function RevenueHero({ daily, loading, onClick, isDark }) {
  const ref = useRef(null)
  useEffect(() => {
    gsap.fromTo(ref.current, { opacity: 0, y: 16, scale: 0.97 }, { opacity: 1, y: 0, scale: 1, duration: 0.6, ease: "back.out(1.5)" })
  }, [])
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.16)", borderRadius: 8, padding: "3px 11px" }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, color: "#fff", fontFamily: "'DM Sans', sans-serif", letterSpacing: "0.3px" }}>Today's Revenue</span>
              </div>
              <ArrowUpRight size={16} color="rgba(255,255,255,0.6)" />
            </div>
            <p style={{ fontSize: 38, fontWeight: 900, margin: "0 0 3px", lineHeight: 1, color: "#fff", fontFamily: "'DM Sans', sans-serif", letterSpacing: "-1.8px" }}>
              ₹{loading ? "—" : (daily?.revenue ?? 0).toLocaleString("en-IN")}
            </p>
            <p style={{ fontSize: 12, margin: "0 0 16px", color: "rgba(255,255,255,0.68)", fontFamily: "'DM Sans', sans-serif" }}>
              {daily?.orders ?? 0} orders · avg ₹{daily?.avgOrderValue ?? 0}
            </p>
            {/* Progress bar */}
            ...
            <p style={{ fontSize: 9, margin: "5px 0 0", color: "rgba(255,255,255,0.45)", fontFamily: "'DM Sans', sans-serif" }}>{daily?.orders ?? 0} / 50 daily target</p>
          </div>
        </div>
      </CometCard>
    </div>
    ...
    <CometCard depth={5} style={{ ...cardBase(isDark), overflow: "hidden" }}>
      <div onClick={onClick} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 15px", cursor: "pointer", borderBottom: `1px solid ${div(isDark)}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, margin: 0, color: tp(isDark), fontFamily: "'DM Sans', sans-serif" }}>Recent Orders</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          ...
          <span style={{ fontSize: 10, color: P.green, fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>Live</span>
        </div>
      </div>
        ...
        {loading
          ...
          : orders.length === 0
            ? <div style={{ padding: "26px 16px", textAlign: "center" }}>
              ...
              <p style={{ fontSize: 12, color: ts(isDark), margin: 0, fontFamily: "'DM Sans', sans-serif" }}>No orders today yet</p>
            </div>
            : orders.slice(0, 12).map((o, i) => {
              ...
              return (
                <div key={o._id ?? i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 15px", borderBottom: `1px solid ${div(isDark)}`, animation: `kcFadeIn 0.3s ease ${i*0.05}s both` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 12, fontWeight: 700, margin: 0, color: tp(isDark), fontFamily: "'DM Sans', sans-serif" }}>Table {o.tableNumber ?? o.table?.tableNumber ?? "?"}</p>
                    <p style={{ fontSize: 10, margin: "1px 0 0", color: sc, fontFamily: "'DM Sans', sans-serif", textTransform: "capitalize", fontWeight: 600 }}>{o.status ?? "pending"}</p>
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 800, color: tp(isDark), fontFamily: "'DM Sans', sans-serif", letterSpacing: "-0.3px" }}>₹{(o.total ?? o.totalAmount ?? 0).toLocaleString("en-IN")}</span>
                </div>
              )
            })
        }
    ...
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${cols},1fr)`, gap: 9 }}>
      {btns.map(({ label, Icon, color, tab }, i) => (
        <CometCard key={tab} depth={9} style={{ ...cardBase(isDark), animation: `kcFadeIn 0.35s ease ${i*0.07}s both`, overflow: "hidden" }}>
          ...
          <button onClick={() => go(tab)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "12px 8px", background: "transparent", border: "none", color, fontSize: 11, fontWeight: 800, fontFamily: "'DM Sans', sans-serif", cursor: "pointer", letterSpacing: "0.3px" }}>
...
function RightSidebar({ daily, orders, loading, isDark, go }) {
  const ref = useRef(null)
  useEffect(() => {
    gsap.fromTo(ref.current, { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.55, delay: 0.2, ease: "expo.out" })
  }, [])
        ...
        {[
          ...
          <CometCard key={label} depth={7} style={{ ...cardBase(isDark) }}>
            <div style={{ padding: "13px 13px 12px" }}>
              ...
              <p style={{ fontSize: 24, fontWeight: 900, margin: "0 0 2px", letterSpacing: "-0.8px", color: tp(isDark), fontFamily: "'DM Sans', sans-serif", lineHeight: 1 }}>
                {raw ? val : <AnimNum value={val} />}
              </p>
              <p style={{ fontSize: 9, margin: 0, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.8px", color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>{label}</p>
            </div>
          </CometCard>
        ))}
      </div>
...
// ─── Page background — near-black with subtle warm orange ambience ─────────────
function Background({ isDark }) {
  ...
  // Dark: near-black like reference (#0D0B09), barely perceptible warm glow
  const base = isDark
    ? "#0D0B09"
    : "linear-gradient(155deg, #E8DDD0 0%, #EDE3D5 50%, #E4D8C8 100%)"
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: mobile ? 20 : 24, overflowY: "auto", scrollbarWidth: "none", paddingBottom: mobile ? 90 : 32 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: mobile ? 21 : 26, fontWeight: 900, margin: "0 0 3px", lineHeight: 1.1, letterSpacing: "-0.6px", color: isDark ? P.textAcc : "#6B2600" }}>
                {greet()}, {user?.name?.split(" ")[0] || "Manager"} 👋
              </h1>
              <p style={{ fontSize: 11, margin: 0, color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>
                {new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long" })} · Kausī Chiyā
              </p>
            </div>
          </div>
          {/* Date range pill — mirrors reference */}
          {!mobile && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 14px", borderRadius: 10, background: bg(isDark), border: `1px solid ${bdr(isDark)}` }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, color: ts(isDark), fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: mobile ? 9 : 12 }}>
            <StatCard label="7D Revenue"   value={sales?.totalRevenue ?? 0} prefix="₹"  icon={TrendingUp}  color={P.orange}  isDark={isDark} delay={0.04} sub={`${sales?.totalOrders ?? 0} orders`}         graphData={revData}  graphPrefix="₹" onClick={() => go("reports")} />
            <StatCard label="Avg Order"    value={sales?.avgOrder ?? 0}     prefix="₹"  icon={ShoppingBag} color="#F97316"   isDark={isDark} delay={0.09} graphData={avgData} graphPrefix="₹"                 onClick={() => go("reports")} />
            <StatCard label="Orders Today" value={daily?.orders ?? 0}                   icon={Activity}    color={P.green}   isDark={isDark} delay={0.14} sub={`avg ₹${daily?.avgOrderValue ?? 0}`}           graphData={ordData}  onClick={() => go("reports")} />
...
function Section({ active, go }) {
  ...
  useEffect(() => {
    if (!ref.current) return
    gsap.fromTo(ref.current, { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.30, ease: "expo.out" })
  }, [active])
...
export default function ManagerDashboard() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;0,800;0,900;1,700&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600;9..40,700;9..40,800;9..40,900&family=DM+Mono:wght@400;500&display=swap');
      ...
      <DashboardLayout role="manager" title="कौसी चिया" navItems={NAV} activeNav={active} onNavChange={go}>
