...
const StaffLoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (logoRef.current)  tl.fromTo(logoRef.current,  { y: -28, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7 }, 0)
    if (panelRef.current) tl.fromTo(panelRef.current, { y: 44,  opacity: 0 }, { y: 0, opacity: 1, duration: 0.65 }, 0.18)
    if (formRef.current)  tl.fromTo(formRef.current,  { y: 18,  opacity: 0 }, { y: 0, opacity: 1, duration: 0.5  }, 0.38)
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#07090B', position: 'relative', overflow: 'hidden', padding: '24px 18px', fontFamily: '"DM Sans", system-ui, sans-serif' }}>
      <style>{`
        ...
        .sl-input-filled { background:rgba(56,189,248,0.04); color:#E8F0F8; border:1px solid rgba(56,189,248,0.2); box-shadow:inset 0 1px 3px rgba(0,0,0,0.3); }
        .sl-input-filled::placeholder { color:rgba(148,183,210,0.3); }
        .sl-input-filled:focus { border-color:rgba(56,189,248,0.5); box-shadow:0 0 0 3px rgba(56,189,248,0.1),inset 0 1px 3px rgba(0,0,0,0.3); }
        .sl-input-error  { border-color:rgba(244,63,94,0.45) !important; box-shadow:0 0 0 3px rgba(244,63,94,0.09) !important; }
      ...
      {/* Grid bg */}
      <div aria-hidden style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', backgroundImage: `linear-gradient(rgba(56,189,248,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(56,189,248,0.04) 1px,transparent 1px)`, backgroundSize: '44px 44px', animation: 'sl-grid 7s ease-in-out infinite' }}/>
      <div aria-hidden style={{ position: 'fixed', top: '-12%', right: '-10%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle,rgba(56,189,248,0.11) 0%,transparent 68%)', filter: 'blur(52px)', pointerEvents: 'none', zIndex: 0, animation: 'sl-pulse 5s ease-in-out infinite' }}/>
      <div aria-hidden style={{ position: 'fixed', bottom: '-14%', left: '-12%', width: 340, height: 340, borderRadius: '50%', background: 'radial-gradient(circle,rgba(167,139,250,0.09) 0%,transparent 68%)', filter: 'blur(48px)', pointerEvents: 'none', zIndex: 0 }}/>
      ...
      <div ref={logoRef} style={{ textAlign: 'center', marginBottom: 28, position: 'relative', zIndex: 10 }}>
        <div style={{ position: 'relative', width: 64, height: 64, margin: '0 auto 14px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ position: 'absolute', inset: -8, borderRadius: '50%', background: 'radial-gradient(circle,rgba(56,189,248,0.2) 0%,transparent 70%)', filter: 'blur(12px)', animation: 'sl-pulse 3s ease-in-out infinite' }}/>
          <div style={{ width: 64, height: 64, borderRadius: 18, background: 'linear-gradient(135deg,rgba(56,189,248,0.14) 0%,rgba(56,189,248,0.04) 100%)', border: '1px solid rgba(56,189,248,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
      ...
      {/* Panel */}
      <div ref={panelRef} style={{ width: '100%', maxWidth: 360, position: 'relative', zIndex: 10, borderRadius: 22, overflow: 'hidden', background: 'rgba(9,13,17,0.9)', backdropFilter: 'blur(40px) saturate(140%)', WebkitBackdropFilter: 'blur(40px) saturate(140%)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 32px 80px rgba(0,0,0,0.75),0 0 0 1px rgba(56,189,248,0.05)' }}>
        <div ref={scanLineRef} aria-hidden style={{ position: 'absolute', left: 0, right: 0, height: 1, top: '0%', background: 'linear-gradient(90deg,transparent 0%,rgba(56,189,248,0.55) 50%,transparent 100%)', pointerEvents: 'none', zIndex: 20 }}/>
        <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: 1, borderRadius: 99, background: 'linear-gradient(90deg,transparent,rgba(56,189,248,0.45) 40%,rgba(167,139,250,0.35) 60%,transparent)' }}/>
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  disabled={isBusy || !!grantedRole}
                />
                <button type="button" tabIndex={-1} onClick={() => setShowPass(v => !v)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', padding: 4, cursor: 'pointer', color: 'rgba(148,183,210,0.32)', display: 'flex', alignItems: 'center' }}>
