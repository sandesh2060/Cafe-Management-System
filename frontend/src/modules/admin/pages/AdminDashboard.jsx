          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard
                ...
                color={COLORS.brew.DEFAULT}
              />
              <StatCard
                icon={Activity}
                label="Orders Today"
                value={usage?.ordersToday}
              ...
              {loading ? (
                <div className="p-4 space-y-2">
                  {[...Array(5)].map((_, i) => (
                    <div
                      key={i}
                      className="h-12 bg-cream-deep rounded-xl animate-pulse"
                    />
                  ))}
                </div>
              ) : (
                  ...
                  {users.map((u) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${u.isActive ? "bg-matcha/10 text-matcha" : "bg-red-100 text-red-500"}`}
                        >
                        ...
                        <button onClick={() => toggleUser(u._id, u.isActive)}>
                          {u.isActive ? (
                            ...
                            <ToggleLeft size={20} color={COLORS.brew.soft} />
                          )}
                        </button>
                      </div>
                    </div>
      ...
      <div className="divide-y divide-gray-50 max-h-[60vh] overflow-auto">
        {orders.map((o) => (
          <div key={o._id} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-brew">
            ...
            <div className="text-right">
              <p className="font-bold text-brew text-sm">Rs {o.total}</p>
              <p className="text-xs text-brew-soft capitalize">{o.status}</p>
            </div>
          </div>
        ))}
        {orders.length === 0 && (
          <div className="py-8 text-center text-brew-soft text-sm">
            No activity
          </div>
        )}
      </div>
