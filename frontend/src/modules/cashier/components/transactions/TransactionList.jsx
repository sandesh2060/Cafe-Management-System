...
const TransactionList = () => {
  ...
  return (
    ...
    >
      {/* Header + summary */}
      <div
        className={`px-4 py-3 border-b ${dk ? "border-gray-800" : "border-gray-100"}`}
      >
      ...
      {loading ? (
        <div className="p-4 space-y-2">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className={`h-12 rounded-xl animate-pulse ${dk ? "bg-gray-800" : "bg-gray-100"}`}
            />
          ))}
        </div>
      ) : txns.length === 0 ? (
        ...
        <div
          className="divide-y max-h-64 overflow-auto"
          style={{ borderColor: dk ? "rgba(255,255,255,0.05)" : "#f9fafb" }}
        >
          {txns.map((t) => {
            const m = METHOD_META[t.paymentMethod] || {
              label: "—",
              color: "#6B7280",
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-semibold ${dk ? "text-white" : "text-gray-900"}`}
                  >
                    Table #{t.tableNumber || "?"}
                  </p>
                  <p
                ...
                <p
                  className={`font-black text-sm flex-shrink-0 ${dk ? "text-white" : "text-gray-900"}`}
                >
                  Rs {t.total}
                </p>
              </div>
