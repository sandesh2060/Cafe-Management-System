...
//
// FIXES:
// • goToMenu useCallback dep array now includes `session` — was stale closure,
//   session was always null inside the callback after login.
// • isSubmitting reset moved before setTimeout in success path — button
//   re-enables immediately after burst, not after 900ms navigation delay.
// • checkUsername loading flag no longer blocks the submit button — used
//   a separate `isChecking` state so the button stays responsive during debounce.
...
import {
  UserRound, ArrowRight, AtSign, Sparkles, Check, X, Shield,
} from 'lucide-react'
...
if (import.meta.env.DEV && !import.meta.env.VITE_CAFE_ID) {
  console.warn('[LoginPage] VITE_CAFE_ID not set — using "demo". Add it to .env.local.')
}
...
const AnimatedLogo = ({ isDark: D }) => (
  <>
    <style>{`
      ...
      .lp-input{width:100%;padding:14px 44px 14px 42px;border-radius:14px;font-family:"Baloo 2",system-ui,sans-serif;font-weight:500;font-size:15px;box-sizing:border-box;outline:none;transition:border-color 0.25s,box-shadow 0.25s,background 0.25s;-webkit-tap-highlight-color:transparent;}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 22 }}>
      ...
      <div style={{ textAlign: 'center', marginTop: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
          ...
          <p className="lp-subtitle" style={{ fontFamily: '"Baloo 2",system-ui,sans-serif', fontSize: 10, fontWeight: 700, letterSpacing: '0.24em', textTransform: 'uppercase', color: D ? 'rgba(255,200,90,0.65)' : 'rgba(150,80,10,0.72)', margin: 0 }}>Smart Café · Kathmandu</p>
...
const UsernameBadge = ({ status, D }) => {
  ...
  const cfg = {
    ...
    exists:   { bg: D ? 'rgba(34,197,94,0.1)' : 'rgba(21,128,61,0.08)', border: D ? 'rgba(34,197,94,0.3)' : 'rgba(21,128,61,0.2)', color: D ? '#4ade80' : '#15803d', icon: <Check size={11} strokeWidth={3}/>, text: 'Found — signing you in!' },
  ...
  if (!cfg) return null
  return (
    <div key={status} className="lp-badge-in" style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 11px', borderRadius: 10, background: cfg.bg, border: `1px solid ${cfg.border}`, color: cfg.color, fontSize: 12, fontWeight: 600, fontFamily: '"Baloo 2",system-ui,sans-serif', marginBottom: 12 }}>
      {cfg.icon}{cfg.text}
    </div>
  )
}
...
const SuccessBurst = ({ show, D }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
      {[...Array(8)].map((_, i) => (
        <div key={i} style={{ position: 'absolute', width: 8, height: 8, borderRadius: '50%', background: i % 2 === 0 ? '#FF9F1C' : '#22c55e', top: '50%', left: '50%', transform: `rotate(${i * 45}deg) translateY(-60px)`, animation: `lp-success-burst 0.7s ease-out ${i * 0.05}s both` }}/>
      ))}
...
const LoginPage = () => {
  ...
  const session     = useSelector(selectSession)      // FIX: read at top level
  ...
  /* ── Navigate helper ── */
  // FIX: session added to dep array — was stale closure, session was always null inside
  ...
  const handleSubmit = async (e) => {
    ...
    // Use cached check result if it matches what the user typed
    let exists = null
    ...
    if (exists) {
      ...
      // FIX: reset isSubmitting BEFORE the setTimeout so button re-enables
      setIsSubmitting(false)
      setShowBurst(true)
      ...
      // FIX: reset isSubmitting BEFORE the setTimeout
      setIsSubmitting(false)
      setShowBurst(true)
  ...
  const iconColor =
    usernameStatus === 'exists'  ? (D ? '#4ade80' : '#15803d')
    : usernameStatus === 'free'  ? (D ? '#93c5fd' : '#2563eb')
    : D ? 'rgba(255,159,28,0.55)' : 'rgba(180,100,20,0.6)'

  const btnBg =
    usernameStatus === 'exists' ? 'linear-gradient(135deg,#22c55e 0%,#16a34a 100%)'
    : usernameStatus === 'free' ? 'linear-gradient(135deg,#60a5fa 0%,#2563eb 100%)'
    : 'linear-gradient(135deg,#FF9F1C 0%,#E8612A 100%)'

  const btnShadow =
    usernameStatus === 'exists' ? '0 6px 28px rgba(34,197,94,0.4)'
    : usernameStatus === 'free' ? '0 6px 28px rgba(96,165,250,0.4)'
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', minHeight: '100dvh', padding: '0 18px' }}>
        ...
        <div ref={cardRef} style={{ marginBottom: 'max(28px,calc(env(safe-area-inset-bottom) + 20px))', borderRadius: 28, overflow: 'hidden', position: 'relative', background: D ? 'rgba(9,5,1,0.82)' : 'rgba(250,246,234,0.86)', backdropFilter: 'blur(42px) saturate(165%)', WebkitBackdropFilter: 'blur(42px) saturate(165%)', border: D ? '1px solid rgba(255,159,28,0.14)' : '1px solid rgba(255,255,255,0.72)', boxShadow: D ? '0 -1px 0 rgba(255,255,255,0.06) inset,0 32px 80px rgba(0,0,0,0.7)' : '0 -1px 0 rgba(255,255,255,0.95) inset,0 24px 60px rgba(92,51,23,0.24)', padding: '26px 22px 24px' }}>
          ...
          {tableNumber && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 18, padding: '9px 16px', borderRadius: 14, background: D ? 'rgba(255,159,28,0.08)' : 'rgba(255,159,28,0.1)', border: `1px solid ${D ? 'rgba(255,159,28,0.2)' : 'rgba(255,159,28,0.26)'}` }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: D ? '#FFB84D' : '#C8680A', fontFamily: '"Baloo 2",system-ui,sans-serif' }}>Table {tableNumber} is ready for you</span>
            </div>
          )}

          {displayError && (
            <div className="lp-slide-up" style={{ marginBottom: 14, padding: '10px 14px', borderRadius: 12, background: 'rgba(220,38,38,0.07)', border: '1px solid rgba(220,38,38,0.2)', color: D ? '#FCA5A5' : '#991B1B', fontSize: 13, fontFamily: '"Baloo 2",system-ui,sans-serif' }}>
              {displayError}
            </div>
          )}

          <div style={{ textAlign: 'center', marginBottom: 22 }}>
            <h2 style={{ fontFamily: '"Noto Sans Devanagari","Baloo 2",system-ui,sans-serif', fontWeight: 800, fontSize: 'clamp(21px,5.5vw,25px)', letterSpacing: '-0.03em', color: D ? '#FFF8EE' : '#120D06', margin: '0 0 5px', lineHeight: 1.2 }}>स्वागत छ! 🙏</h2>
            <p style={{ fontFamily: '"Baloo 2",system-ui,sans-serif', fontSize: 13, color: D ? 'rgba(240,196,110,0.52)' : 'rgba(92,51,23,0.5)', margin: 0 }}>Enter your username to sign in or register</p>
          </div>

          <form onSubmit={handleSubmit} autoComplete="off">
            <label style={{ fontFamily: '"Baloo 2",system-ui,sans-serif', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: D ? 'rgba(255,200,90,0.55)' : 'rgba(150,80,10,0.65)', marginBottom: 7, display: 'block' }}>Username</label>
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '15px 18px', borderRadius: 16, marginBottom: 14, border: 'none', background: !canSubmit ? (D ? 'rgba(255,159,28,0.2)' : 'rgba(200,104,10,0.15)') : btnBg, color: '#FFFFFF', fontFamily: '"Baloo 2",system-ui,sans-serif', fontWeight: 700, fontSize: 15, minHeight: 52, boxShadow: !canSubmit ? 'none' : btnShadow, transition: 'all 0.3s cubic-bezier(.22,.68,0,1.2)', WebkitTapHighlightColor: 'transparent', position: 'relative', overflow: 'hidden', cursor: canSubmit ? 'pointer' : 'not-allowed', opacity: !canSubmit ? 0.45 : 1 }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '0 0 14px' }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.07em', color: D ? 'rgba(255,196,100,0.3)' : 'rgba(92,51,23,0.32)', fontFamily: '"Baloo 2",system-ui,sans-serif' }}>OR</span>
            <div style={{ flex: 1, height: 1, background: D ? 'rgba(255,255,255,0.07)' : 'rgba(92,51,23,0.1)' }}/>
          </div>

          <button onClick={handleGuest} disabled={isLoading || guestLoading} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '13px 18px', borderRadius: 16, marginBottom: 12, border: D ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.07)', background: D ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.88)', color: D ? '#FFF8EE' : '#1A1208', fontFamily: '"Baloo 2",system-ui,sans-serif', fontWeight: 600, fontSize: 14, cursor: 'pointer', minHeight: 48, opacity: (isLoading || guestLoading) ? 0.55 : 1, transition: 'opacity 0.15s', WebkitTapHighlightColor: 'transparent' }}>
            <UserRound size={16} strokeWidth={2.2}/>
            {guestLoading ? 'Setting up…' : 'Continue as Guest'}
          </button>

          <button onClick={handleStaffLogin} disabled={isLoading} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 18px', borderRadius: 16, marginBottom: 16, border: D ? '1px solid rgba(255,159,28,0.15)' : '1px solid rgba(180,100,20,0.18)', background: 'transparent', color: D ? 'rgba(255,184,77,0.7)' : 'rgba(150,80,10,0.65)', fontFamily: '"Baloo 2",system-ui,sans-serif', fontWeight: 600, fontSize: 13, cursor: 'pointer', minHeight: 42, opacity: isLoading ? 0.45 : 1, transition: 'opacity 0.15s, background 0.15s', WebkitTapHighlightColor: 'transparent' }} onMouseEnter={e => { e.currentTarget.style.background = D ? 'rgba(255,159,28,0.07)' : 'rgba(180,100,20,0.06)' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 13px', borderRadius: 14, background: D ? 'rgba(255,159,28,0.06)' : 'rgba(255,242,205,0.8)', border: D ? '1px solid rgba(255,159,28,0.12)' : '1px solid rgba(228,182,78,0.35)' }}>
            ...
            <div>
              <p style={{ fontFamily: '"Baloo 2",system-ui,sans-serif', fontWeight: 700, fontSize: 12, color: D ? '#FFB84D' : '#7A4A0A', margin: '0 0 2px' }}>Sign in to earn loyalty points</p>
              <p style={{ fontFamily: '"Baloo 2",system-ui,sans-serif', fontSize: 11, color: D ? 'rgba(255,175,60,0.5)' : 'rgba(122,74,10,0.55)', margin: 0, lineHeight: 1.4 }}>Bronze → Silver → Gold · Up to 15% off</p>
            </div>
          </div>
        </div>
      </div>
