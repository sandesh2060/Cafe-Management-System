...
//
// FIXES:
//   • "Plus Jakarta Sans" → "Baloo 2" in textarea and button inline fontFamily
//   • window.__sheetOpen = true/false so NotificationToast suppresses itself
//     while the cart is open (prevents toast overlapping the drawer)
...
import {
  selectTableId, selectSessionId,
} from "@store/slices/tableSessionSlice";

import {
  placeOrder, selectOrderPlacing, selectOrderError,
  selectHasActiveOrder, selectActiveOrder,
  clearError, clearMerged,
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.portionLabel && (
          <p className="m-0 text-[10px] mt-0.5 font-semibold"
             style={{ color: D ? "rgba(255,184,77,0.55)" : "rgba(92,51,23,0.5)" }}>
            {item.portionLabel}
          </p>
        )}
      ...
      <div className="flex flex-col items-end gap-2">
        <button
          ...
          style={{
            background: D ? "rgba(255,255,255,0.06)" : "rgba(92,51,23,0.06)",
            color: "#ef4444",
...
const CartDrawer = ({ open, onClose }) => {
  ...
  // ── Suppress NotificationToast while drawer is open ──────────────────────
  useEffect(() => {
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-0 right-0 bottom-0 z-[9081] flex flex-col"
            style={{
              background: bg,
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 pt-2 pb-3 flex-shrink-0"
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="m-0 text-[16px] font-extrabold" style={{ color: text }}>
                    {isAddon ? "Add to Order" : "Your Cart"}
                  </h2>
                  <p className="m-0 text-[10px]" style={{ color: muted }}>
              ...
              {items.length === 0 ? (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <p className="m-0 text-[12px]"
                     style={{ color: D ? "rgba(196,154,108,0.5)" : "rgba(139,94,60,0.45)" }}>
                ...
                <div className="mt-4 mb-2">
                  <textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)}
                    ...
                    style={{
                      ...
                      fontSize: 12, lineHeight: 1.6, borderRadius: 13, padding: "9px 13px",
                    ...
                    onBlur={(e)  => { e.target.style.borderColor = D ? "rgba(255,159,28,0.15)" : "rgba(200,175,135,0.5)"; e.target.style.boxShadow = "none"; }}
                  />
                </div>
              )}
            </div>
            ...
            {/* Footer */}
            {items.length > 0 && (
              <div className="flex-shrink-0 px-5 pt-3 pb-5" style={{ borderTop: `1px solid ${border}` }}>
                <div className="mb-3 space-y-1.5">
                  ...
                  {discount > 0 && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                      ...
                      <span className="font-mono font-bold" style={{ color: "#22c55e" }}>−Rs {discount}</span>
                    </motion.div>
                  )}
                  <div className="flex justify-between text-[15px] font-extrabold pt-1"
                       style={{ borderTop: `1px solid ${border}`, paddingTop: 8, color: text }}>
                    <span>Total</span>
                    <span className="font-mono" style={{ color: D ? "#FFB84D" : "#C8680A" }}>Rs {total}</span>
                  </div>
                </div>
                  ...
                  {isAddon && !orderError && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                      ...
                      <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl"
                           style={{ background: D ? "rgba(255,159,28,0.08)" : "rgba(255,240,200,0.7)", border: `1px solid ${D ? "rgba(255,159,28,0.22)" : "rgba(255,159,28,0.35)"}` }}>
                        <PlusCircle size={13} color="#FF9F1C" strokeWidth={2} className="mt-0.5 flex-shrink-0" />
                        <div className="flex-1">
                          <p className="m-0 text-[12px] font-bold" style={{ color: D ? "#FFB84D" : "#C8680A" }}>
                            Adding to your current order
                          </p>
                          <p className="m-0 text-[11px] mt-0.5" style={{ color: muted }}>
                            You already have {activeOrder?.items?.length ?? "?"} item
                            {(activeOrder?.items?.length ?? 1) !== 1 ? "s" : ""} in your order ({activeOrder?.status}). These will be added on.
                          </p>
                        </div>
                      </div>
                    </motion.div>
                  ...
                  {orderError && (
                    <motion.div initial={{ opacity: 0, y: -6, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }}
                      ...
                      <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl"
                        ...
                        <AlertCircle size={13} color="#ef4444" strokeWidth={2} className="mt-0.5 flex-shrink-0" />
                        <p className="m-0 text-[12px] font-semibold flex-1" style={{ color: "#ef4444" }}>{orderError}</p>
                ...
                <button
                  ...
                  aria-label={placing ? "Placing order…" : isAddon ? `Add items to order · Rs ${total}` : `Place order · Rs ${total}`}
                  ...
                  style={{
                    background: placing ? (D ? "rgba(255,159,28,0.35)" : "rgba(255,159,28,0.45)") : "linear-gradient(135deg,#FF9F1C,#E05C2A)",
                ...
                >
                  {placing ? (
                    <><Loader2 size={17} strokeWidth={2.5} style={{ animation: "cd-spin 0.7s linear infinite" }} />
                      {isAddon ? "Adding to Order…" : "Placing Order…"}</>
                  ) : isAddon ? (
                    <><PlusCircle size={16} strokeWidth={2.5} />Add to Order · Rs {total}</>
                  ) : (
                    <>Place Order · Rs {total}<ChevronRight size={16} strokeWidth={2.5} /></>
                  )}
                </button>
              </div>
            )}
