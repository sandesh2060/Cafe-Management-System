...
const SlideDecor = ({ accent, index }) => {
  if (index === 0) return (
    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 360 148">
      <defs>
      ...
      {[50,80,110,145].map((r,i) => (
        <circle key={i} cx="88%" cy="50%" r={r} fill="none"
          stroke={accent} strokeOpacity={0.1 - i*0.02} strokeWidth="1"/>
      ))}
    </svg>
  )
  if (index === 1) return (
    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 360 148">
      ...
      <rect x="220" y="0" width="140" height="148" fill={accent} fillOpacity="0.07"/>
    </svg>
  )
  return (
    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 360 148">
      ...
      <path d="M240,0 Q300,74 240,148 L360,148 L360,0 Z" fill={accent} fillOpacity="0.07"/>
    </svg>
  )
}
  ...
  const enter = useCallback(() => {
    ...
    tlRef.current
      .fromTo(orbRef.current,
        ...
        { scale: 1,   opacity: 1, rotation: 0,   x: 0,
          duration: 0.62, ease: 'back.out(2.4)' }
      )
      .fromTo(linesRef.current?.children ? Array.from(linesRef.current.children) : [],
        { x: -28, opacity: 0 },
    ...
    if (floatRef.current) {
      ...
      gsap.fromTo(nodes,
        ...
        { y:  0, opacity: 1, scale: 1,   rotation:  0,
          duration: 0.48, stagger: 0.09, delay: 0.25, ease: 'back.out(2.2)' }
      )
      gsap.to(nodes, {
        y: '-=9', duration: 2.8, repeat: -1, yoyo: true,
        stagger: 0.6, ease: 'sine.inOut', delay: 0.85,
      })
    }

    // Orb breathe
    gsap.to(orbRef.current, {
      scale: 1.05, duration: 2.4, repeat: -1, yoyo: true,
  ...
  return (
    <div className="relative overflow-hidden select-none"
      ...
      <div ref={floatRef}
        ...
        aria-hidden="true">
        {s.floats.map((f,i) => (
          <span key={i} className="text-base leading-snug opacity-0"
      ...
      <div className="relative z-10 h-full flex items-center px-5 gap-5">
        ...
        <div ref={linesRef} className="flex flex-col min-w-0 gap-0">
          {/* Pill tag */}
          <span className="self-start mb-2 px-2.5 py-[3px] rounded-full opacity-0
          ...
          <h3 className="font-black leading-[1.1] opacity-0 whitespace-pre-line"
            style={{
              fontSize: '21px',
              color: '#fff',
          ...
          {/* Sub */}
          <p className="mt-1.5 text-[12px] font-semibold opacity-0"
