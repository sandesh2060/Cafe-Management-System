...
const RecommendedCard = ({ rec, index = 0 }) => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(el, { opacity:0, y:20 }, { opacity:1, y:0, duration:0.44, delay:0.05+index*0.07, ease:'power3.out', clearProps:'opacity,y,transform' })
  }, [index])
  ...
  }, [_id, name, price, emoji, category, hasPortions, dispatch])
    ...
    <>
      ...
      >
        ...
        <div style={{ position:'relative', height:130, overflow:'hidden', flexShrink:0, background:S.imageBg }}>
          {image ? (
            <>
              {!imgLoaded && <div style={{ position:'absolute', inset:0, background:`linear-gradient(90deg,${S.imageBg} 0%,rgba(255,255,255,0.3) 50%,${S.imageBg} 100%)`, backgroundSize:'200% 100%', animation:'rc-shimmer 1.5s ease-in-out infinite' }}/>}
              <img ref={imgRef} src={image} alt={name} onLoad={() => setImgLoaded(true)} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block', opacity:imgLoaded?1:0, transition:'opacity 0.3s ease', transform:'translate3d(0,0,0)' }} loading="lazy" draggable={false}/>
            </>
          ) : (
        ...
        <div style={{ padding:'10px 12px 12px', display:'flex', flexDirection:'column', flex:1 }}>
          ...
          {/* Price — from shared PriceDisplay */}
          ...
          {(rating != null || reviewCount > 0) && (
            <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:4 }}>
              {rating != null && <div style={{ display:'flex', alignItems:'center', gap:3 }}><Star size={10} fill="#F59E0B" color="#F59E0B"/><span style={{ fontSize:10, fontWeight:700, color:'#F59E0B', fontFamily:FONTS.mono }}>{rating}</span></div>}
