...
const RecommendedCard = ({ rec, index = 0 }) => {
  ...
  useEffect(() => {
    ...
    // Set initial state via GSAP — NOT via Tailwind class
    // Tailwind opacity-0 has higher CSS specificity and blocks GSAP's inline style
    gsap.set(cardRef.current, { opacity: 0, y: 20, scale: 0.92 })
  ...
  return (
    <div
      ref={cardRef}
      // ✅ NO opacity-0 class — GSAP owns opacity via gsap.set above
      className="w-[140px] flex flex-col rounded-2xl overflow-hidden"
      style={{
        ...
        {weatherTag && (
          <span
            ...
            style={{
              ...
              color:      'var(--color-saffron, #FF9F1C)',
              border:     '1px solid var(--border-color)',
            }}
          >
            {weatherTag}
