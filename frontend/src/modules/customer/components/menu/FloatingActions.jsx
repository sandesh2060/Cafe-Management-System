...
//
// ─── MODULE 22 + REMOTE ORDER INTEGRATION ─────────────────────────────────────
// ★ NEW: accepts `isRemote` prop
// ★ NEW: when isRemote, renders RemoteOrderBanner stacked above the cart bar
// ★ NEW: AddressSheet portal rendered here (removed from MenuPage)
// ★ NEW: bottom content padding accounts for banner height in remote mode
//
// All GSAP animations, scroll hide/show, drawer logic — UNCHANGED
// Order pill, cart bar, shimmer, badge pop — UNCHANGED
// ─────────────────────────────────────────────────────────────────────────────
...
import RemoteOrderBanner  from './RemoteOrderBanner'
...
function notifyDrawerChange(open) {
  window.__drawerOpen = open
  window.dispatchEvent(new CustomEvent('drawerchange', { detail: { open } }))
}

// ★ Height constants — used for pill offset calculation
const CART_BAR_H   = 82   // cart bar button height + padding
const BANNER_H     = 46   // RemoteOrderBanner height

export default function FloatingActions({ isRemote = false }) {
  ...
  // ── Hide/show on drawer state change ─────────────────────────────────────
  useEffect(() => {
    const bar  = cartBarRef.current
    const pill = orderPillRef.current
    if (!bar) return
    notifyDrawerChange(drawerOpen)
    if (drawerOpen) {
      gsap.to(bar, { y: 120, opacity: 0, duration: 0.22, ease: 'power3.in', force3D: true, overwrite: true })
      if (pill && pillVisible.current)
        gsap.to(pill, { y: 20, opacity: 0, scale: 0.90, duration: 0.18, ease: 'power3.in', force3D: true, overwrite: true })
    } else {
      if (cartVisible.current && !barHidden.current)
        gsap.fromTo(bar, { y: 120, opacity: 0 }, { y: 0, opacity: 1, duration: 0.42, ease: 'back.out(1.7)', force3D: true, overwrite: true })
      if (pill && pillVisible.current && !barHidden.current)
        gsap.fromTo(pill, { y: 20, opacity: 0, scale: 0.90 }, { y: 0, opacity: 1, scale: 1, duration: 0.40, ease: 'back.out(2)', force3D: true, overwrite: true })
    }
  }, [drawerOpen])

  // ── Cart bar — count changes ──────────────────────────────────────────────
  useEffect(() => {
    const bar = cartBarRef.current
    if (!bar) return
    ...
    if (cartCount === 0) {
      ...
      gsap.to(bar, { y: 120, opacity: 0, scale: 0.95, duration: 0.32, ease: 'power3.in', force3D: true, overwrite: true,
    ...
    if (wasEmpty || barHidden.current) {
      ...
      barHidden.current   = false
      gsap.fromTo(bar, { y: 120, opacity: 0, scale: 0.95 }, {
        y: 0, opacity: 1, scale: 1, duration: 0.58, ease: 'back.out(1.7)', force3D: true, clearProps: 'scale',
        onStart: () => {
          if (wasEmpty && shineRef.current)
            gsap.fromTo(shineRef.current, { x: '-110%' }, { x: '110%', duration: 0.9, ease: 'power2.out', delay: 0.28 })
        },
      })
      return
    }
  ...
  useEffect(() => {
    ...
    if (showPill && !pillVisible.current) {
      ...
      gsap.fromTo(pill, { y: 20, opacity: 0, scale: 0.88 }, { y: 0, opacity: 1, scale: 1, duration: 0.55, ease: 'back.out(2)', force3D: true, overwrite: true })
      if (pillShineRef.current)
        gsap.fromTo(pillShineRef.current, { x: '-110%' }, { x: '110%', duration: 0.85, ease: 'power2.out', delay: 0.35 })
    ...
    if (orderStatus !== prevStatus.current && pillVisible.current && pillLabelRef.current) {
      ...
      gsap.fromTo(pillLabelRef.current, { opacity: 0, x: 8 }, { opacity: 1, x: 0, duration: 0.3, ease: 'power2.out' })
    }
  ...
  useEffect(() => {
    ...
    if (isActive && meta) {
      ringAnimRef.current = gsap.to(ring, { scale: 2.4, opacity: 0, duration: 1.1, repeat: -1, ease: 'power2.out', transformOrigin: 'center' })
    } else {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(badgeRef.current, { scale: 0.4, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.38, ease: 'back.out(3)', overwrite: true })
  }, [cartCount])
  ...
  useEffect(() => {
    ...
    gsap.fromTo(priceRef.current, { y: 8, opacity: 0 }, { y: 0, opacity: 1, duration: 0.28, ease: 'expo.out', overwrite: true })
  }, [cartTotal])
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      gsap.fromTo(shine, { x: '-110%' }, { x: '110%', duration: 2.6, ease: 'none', delay: 5, onComplete: loop })
    }
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (!barHidden.current && !atTop && s.vel > HIDE_VEL) {
        ...
        barHidden.current = false
        s.upDist = 0
        gsap.fromTo(bar,  { y: 120, opacity: 0 }, { y: 0, opacity: 1, duration: 0.42, ease: 'expo.out', force3D: true, overwrite: true })
        if (pill && pillVisible.current)
          gsap.fromTo(pill, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.38, ease: 'back.out(1.8)', force3D: true, overwrite: true })
      }
    }
  ...
  const handleOrderPillTap = useCallback(() => {
    ...
    const route = orderStatus ? getOrderRoute(orderStatus) : '/order/history'
    setTimeout(() => navigate(route), 80)
  }, [navigate, orderStatus])

  // ── Layout heights ────────────────────────────────────────────────────────
  // ★ When remote: cart bar sits above the RemoteOrderBanner
  // Banner is inside the same fixed wrapper so no z-fighting
  const safeBottom = 'env(safe-area-inset-bottom, 0px)'

  // ★ Pill bottom: above cart bar (+ banner if remote)
  const pillBottom = cartCount > 0
    ? isRemote
      ? `calc(${safeBottom} + ${CART_BAR_H}px + ${BANNER_H}px + 10px)`
      : `calc(${safeBottom} + ${CART_BAR_H}px + 10px)`
    : isRemote
      ? `calc(${safeBottom} + ${BANNER_H}px + 20px)`
  ...
  return (
    <>
        ...
        >
          <button
            ...
            style={{
              ...
              background: meta
                ? D ? `linear-gradient(135deg,${meta.color}18,${meta.color}0e)` : `linear-gradient(135deg,${meta.color}14,${meta.color}08)`
              ...
              boxShadow: meta
                ...
                : D ? '0 4px 20px rgba(0,0,0,0.4),0 1px 0 rgba(255,255,255,0.07) inset' : '0 4px 20px rgba(130,80,20,0.12),0 1px 0 rgba(255,255,255,0.9) inset',
          ...
          >
            <div ref={pillShineRef} aria-hidden style={{ position:'absolute',top:0,bottom:0,left:0,width:'45%', background:'linear-gradient(105deg,transparent 0%,rgba(255,255,255,0.06) 35%,rgba(255,255,255,0.16) 50%,rgba(255,255,255,0.06) 65%,transparent 100%)', transform:'translateX(-110%)',pointerEvents:'none' }}/>
            <div style={{ position:'relative', width:10, height:10, flexShrink:0 }}>
              <div ref={pillRingRef} style={{ position:'absolute',inset:0,borderRadius:'50%', background:meta?meta.color:'transparent', transformOrigin:'center',opacity:0 }}/>
              <div style={{ position:'absolute', inset:isActive?2:0, borderRadius:'50%', background:meta?meta.color:D?'rgba(255,184,77,0.5)':'rgba(200,104,10,0.5)', transition:'inset 0.3s ease' }}/>
            </div>
            {meta && <span style={{ fontSize:14, lineHeight:1, flexShrink:0 }}>{meta.emoji}</span>}
            {!meta && hasPast && <History size={13} strokeWidth={2.2} color={D?'rgba(255,184,77,0.6)':'rgba(140,80,20,0.6)'}/>}
            <span ref={pillLabelRef} style={{ fontSize:12, fontWeight:800, fontFamily:'"Baloo 2",system-ui,sans-serif', letterSpacing:'-0.01em', color:meta?meta.color:D?'#FFB84D':'#C8680A' }}>
              {meta ? meta.label : 'Past Orders'}
            </span>
            <ChevronRight size={13} strokeWidth={2.5} color={meta?meta.color+'CC':D?'rgba(255,184,77,0.5)':'rgba(140,80,20,0.45)'}/>
          </button>
        </div>
      )}

      {/* ══ BOTTOM FIXED WRAPPER — cart bar + remote banner stacked ══ */}
      <div
        ref={cartBarRef}
        data-tour="fab"
        className="fixed bottom-0 left-0 right-0 z-[9100] pointer-events-none"
      >
        {/* ★ Remote order banner — sits above the cart bar when isRemote */}
        {isRemote && (
          <div className="pointer-events-auto">
            <RemoteOrderBanner isDark={D} />
          </div>
        )}

        {/* Cart bar */}
        <div className="px-4 pt-3" style={{ paddingBottom: `calc(${safeBottom} + 12px)` }}>
          <button
            onClick={openCart}
            ...
            style={{
              ...
              boxShadow: ['0 8px 40px rgba(255,130,0,0.52)','0 2px 0 rgba(255,255,255,0.22) inset','0 -1px 0 rgba(0,0,0,0.12) inset','0 0 0 1px rgba(255,159,28,0.6)'].join(','),
            }}
          ...
          >
            <div ref={shineRef} aria-hidden className="absolute top-0 bottom-0 left-0 w-[42%] pointer-events-none" style={{ background:'linear-gradient(105deg,transparent 0%,rgba(255,255,255,0.07) 35%,rgba(255,255,255,0.20) 50%,rgba(255,255,255,0.07) 65%,transparent 100%)', transform:'translateX(-110%)' }}/>
            <div aria-hidden className="absolute top-0 left-[10%] right-[10%] h-px pointer-events-none" style={{ background:'linear-gradient(90deg,transparent,rgba(255,255,255,0.38) 30%,rgba(255,255,255,0.55) 50%,rgba(255,255,255,0.38) 70%,transparent)' }}/>

            <div className="flex items-center gap-2.5 flex-shrink-0">
              <div className="relative">
                <div className="w-10 h-10 rounded-[13px] flex items-center justify-center" style={{ background:'rgba(255,255,255,0.18)', border:'1px solid rgba(255,255,255,0.25)', backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)' }}>
                  <ShoppingCart size={19} strokeWidth={2.5} color="#fff"/>
                </div>
                {cartCount > 0 && (
                  <span ref={badgeRef} className="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 rounded-full bg-white text-[#D94F1E] text-[10px] font-black flex items-center justify-center px-1 font-mono leading-none" style={{ boxShadow:'0 2px 8px rgba(0,0,0,0.25)', border:'1.5px solid rgba(255,159,28,0.3)' }}>
                    {cartCount > 99 ? '99+' : cartCount}
                  </span>
                )}
              </div>
              <div className="flex flex-col gap-[1px]">
                <span className="text-[11px] font-bold text-white/75 tracking-[0.04em] uppercase leading-none">{cartCount} item{cartCount !== 1 ? 's' : ''}</span>
                <span className="text-[9px] font-semibold text-white/50 tracking-[0.02em] leading-none">Tap to review</span>
              </div>
            </div>

            <div className="flex-1 flex items-center justify-center gap-1.5">
              <Sparkles size={12} strokeWidth={2} color="rgba(255,255,255,0.7)"/>
              <span className="text-[15px] font-extrabold tracking-[-0.02em] text-white whitespace-nowrap">View Cart</span>
            </div>

            <div className="flex items-center gap-1 flex-shrink-0 overflow-hidden">
              <div className="overflow-hidden" style={{ maxHeight:28 }}>
                <span ref={priceRef} className="block text-[18px] font-black tracking-[-0.04em] text-white leading-none font-mono" style={{ lineHeight:'28px' }}>
      ...
      {/* ★ AddressSheet portal — rendered here, not in MenuPage */}
      {isRemote && <AddressSheet isDark={D} />}
