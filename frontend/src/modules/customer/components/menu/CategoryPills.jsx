...
// 1. gsapEnabled from useDeviceTier() — stagger mount skipped on low tier
// 2. Liquid morphing track + active pill animations — unchanged on mid/high
// 3. Theme color handling via CSS vars — unchanged
// ─────────────────────────────────────────────────────────────────────────────
...
const CategoryPills = ({ categories = [], active = 'all', onChange }) => {
  ...
  const getIdleColor   = () => getComputedStyle(document.documentElement).getPropertyValue('--cp-idle-color').trim()
  const ACTIVE_COLOR = '#ffffff'

  useEffect(() => {
    const pills = Object.values(pillRefs.current).filter(Boolean)
    if (!pills.length) return
    // FIX: skip stagger on low tier — pills appear instantly
    if (!gsapEnabled) return
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return
    gsap.fromTo(pills,
      { y: 16, opacity: 0, scale: 0.8 },
      { y: 0, opacity: 1, scale: 1, duration: 0.42, stagger: 0.055, ease: 'back.out(2.4)', force3D: true, clearProps: 'all', delay: 0.08 }
    )
  }, [categories, gsapEnabled])
  ...
  useLayoutEffect(() => {
    ...
    tl.to(track, { x: stretchX, width: stretchW, duration: 0.2, ease: 'power2.in', force3D: true })
    tl.to(track, { x: targetX, width: targetW, duration: 0.3, ease: 'expo.out', force3D: true }, '-=0.04')

    if (prevActive.current && prevActive.current !== active) {
      const prevEl = pillRefs.current[prevActive.current]
      if (prevEl) tl.to(prevEl, { color: getIdleColor(), duration: 0.2, ease: 'power1.out' }, 0)
    }

    tl.to(activePill, { color: ACTIVE_COLOR, duration: 0.18, ease: 'power2.out' }, 0.14)
    tl.fromTo(activePill, { scale: 0.92 }, { scale: 1, duration: 0.42, ease: 'elastic.out(1.2,0.5)', force3D: true }, 0.16)
  ...
  useEffect(() => {
    ...
    categories.forEach(cat => {
      const el = pillRefs.current[cat]
      if (!el) return
      gsap.to(el, { color: cat === active ? ACTIVE_COLOR : idleColor, duration: 0.22, ease: 'power2.out' })
    })
  }, [isDark])
    ...
    <>
      ...
      <style>{`
        ...
        .cp-scroll {
          ...
          overflow-x:auto;padding:2px 16px 6px;
          -ms-overflow-style:none;scrollbar-width:none;
        }
        .cp-scroll::-webkit-scrollbar{display:none;}
        @media(min-width:640px){.cp-scroll{padding:2px 20px 8px;gap:6px;}}
        @media(min-width:1024px){.cp-scroll{padding:2px 24px 8px;gap:7px;}}
        .cp-track {
          position:absolute;top:2px;left:0;height:calc(100% - 8px);border-radius:100px;
          background:linear-gradient(135deg,#FF9F1C 0%,#E05C2A 100%);
          box-shadow:0 4px 20px rgba(255,130,0,0.38),0 1px 0 rgba(255,255,255,0.22) inset;
          pointer-events:none;z-index:0;will-change:transform,width;transform-origin:left center;
        }
        .cp-pill {
          position:relative;z-index:1;flex-shrink:0;display:flex;align-items:center;gap:5px;
          padding:0 13px;height:34px;border-radius:100px;
          border:1px solid var(--cp-pill-border);background:var(--cp-pill-bg);
        ...
        @media(min-width:640px){.cp-pill{height:36px;padding:0 15px;font-size:13px;}}
        @media(min-width:1024px){.cp-pill{height:38px;padding:0 17px;font-size:13.5px;gap:6px;}}
        .cp-pill:hover{background:var(--cp-pill-hover-bg);border-color:var(--cp-pill-hover-border);}
        .cp-pill:active{transform:scale(0.94);}
        .cp-pill:focus-visible{outline:2px solid rgba(255,159,28,0.5);outline-offset:2px;}
        .cp-icon{font-size:13px;line-height:1;}
        .cp-text{line-height:1;}
        .cp-edge-fade {
          position:absolute;top:0;right:0;width:48px;height:100%;
          background:linear-gradient(to left,var(--cp-edge-bg-start) 0%,transparent 100%);
          pointer-events:none;z-index:5;
          transition:background var(--transition-theme,300ms ease);
        }
        @media(prefers-reduced-motion:reduce){.cp-pill{transition:none;}.cp-edge-fade{transition:none;}}
      `}</style>
    </>
  )
}
