...
const SkeletonCard = () => (
  <div style={{
    ...
    <div style={{ height: 116, background: 'var(--bg-surface-2,#FFE4B5)', position: 'relative', overflow: 'hidden' }}>
      <div style={{
    ...
    <div style={{ padding: '10px 11px 11px', display: 'flex', flexDirection: 'column', gap: 7 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:4 }}>
        ...
        <div style={{ width:32, height:32, borderRadius:10, background:'var(--bg-surface-2,#FFE4B5)' }} />
      </div>
    </div>
  </div>
)
...
const Carousel = ({ items, weather, accent }) => {
  ...
  const D = useRef({
    x: 0, idx: 0, dragging: false,
    startX: 0, startY: 0, prevX: 0, prevY: 0,
    vel: 0, isScroll: null, momentum: false, n: 0,
  })
  ...
  const applyScale = useCallback((idx) => {
    ...
    Array.from(trackRef.current.children).forEach((child, i) => {
      gsap.to(child, {
        ...
        opacity: i === idx ? 1    : 0.80,
        duration: 0.36, ease: 'back.out(1.6)', overwrite: true,
      })
    })
  }, [])

  const clampX = useCallback((x) => {
    const max = 0, min = -(n - 1) * STRIDE
  ...
  const snapTo = useCallback((idx, dur = 0.44) => {
    ...
    gsap.to({ v: D.current.x }, {
      v: target, duration: dur, ease: 'expo.out',
      onUpdate: function() { setTranslate(this.targets()[0].v) },
      onComplete: () => { setTranslate(target); setActiveIdx(clamped); applyScale(clamped) },
    })
  ...
  useLayoutEffect(() => { setTranslate(0); applyScale(0) }, [n, setTranslate, applyScale])

  useEffect(() => {
    const t = setTimeout(() => startAuto(), 1400)
    return () => { clearTimeout(t); stopAuto(); cancelAnimationFrame(rafRef.current) }
  }, [startAuto, stopAuto])
  ...
  useEffect(() => {
    ...
    function onTouchStart(e) {
      stopAuto(); cancelAnimationFrame(rafRef.current)
      D.current.momentum = false
      const t = e.touches[0]
      Object.assign(D.current, { dragging:true, startX:t.clientX, startY:t.clientY, prevX:t.clientX, prevY:t.clientY, vel:0, isScroll:null })
    }
    ...
    function onTouchEnd() {
      ...
      if (Math.abs(vel) > 2) {
        ...
        function step() {
          ...
          v *= FRICTION
          setTranslate(clampX(D.current.x + v))
          if (Math.abs(v) < MIN_VEL) {
            D.current.momentum = false
            snapTo(Math.round(-D.current.x / STRIDE))
      ...
      >
        {items.map((item, i) => (
          <div key={item._id || item.id || i}
            style={{ flexShrink:0, width:CARD_W, willChange:'transform,opacity', transformOrigin:'center bottom' }}
          >
    ...
    <section style={{ marginTop:20 }}>
      ...
      <div ref={headerRef} style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:3, height:18, borderRadius:99, flexShrink:0, background:'linear-gradient(180deg,#FF9F1C,#E05C2A)' }} />
          <h2 style={{ fontSize:14, fontWeight:900, margin:0, lineHeight:1, color:'var(--text-primary,#2C1810)', letterSpacing:'-0.02em' }}>
            Recommended
          </h2>
