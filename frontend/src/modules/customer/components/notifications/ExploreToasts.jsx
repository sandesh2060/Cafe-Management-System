...
//
// FIXES:
// ✅ All hardcoded "'Baloo 2', system-ui, sans-serif" → FONTS.brand
// ✅ All hardcoded '#FFF8EE' / '#2D1000' text colors → var(--text-primary/secondary)
// ✅ All other logic, GSAP animations, guards unchanged
...
const GallerySVG = ({ accent }) => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.to(svgRef.current, { scale: 1.04, duration: 2.2, ease: 'sine.inOut', yoyo: true, repeat: -1 })
      if (lens1Ref.current) gsap.to(lens1Ref.current, { scale: 1.18, opacity: 0.5, duration: 1.4, ease: 'sine.inOut', yoyo: true, repeat: -1, transformOrigin: 'center' })
      if (lens2Ref.current) gsap.to(lens2Ref.current, { scale: 0.88, opacity: 0.85, duration: 1.8, ease: 'sine.inOut', yoyo: true, repeat: -1, transformOrigin: 'center', delay: 0.3 })
      ...
      callsRef.current.push(gsap.delayedCall(0.4, animPolaroid1), gsap.delayedCall(4.0, animPolaroid1))
      ...
      callsRef.current.push(gsap.delayedCall(1.6, animPolaroid2), gsap.delayedCall(5.2, animPolaroid2))
  ...
  return (
    <svg ref={svgRef} width="52" height="52" viewBox="0 0 52 52" fill="none" style={{ flexShrink: 0, transformOrigin: 'center' }}>
      <rect ref={flashRef} x="0" y="0" width="52" height="52" rx="13" fill="white" opacity="0" style={{ pointerEvents: 'none' }} />
      <rect x="6" y="14" width="40" height="28" rx="6" fill={`${accent}22`} stroke={accent} strokeWidth="1.8" />
      <rect x="18" y="9" width="16" height="8" rx="3" fill={`${accent}18`} stroke={accent} strokeWidth="1.5" />
      <circle cx="38" cy="20" r="2.5" fill={accent} opacity="0.7" />
      <circle cx="24" cy="28" r="10" fill={`${accent}14`} stroke={accent} strokeWidth="1.6" />
      <circle ref={lens1Ref} cx="24" cy="28" r="6.5" fill={`${accent}20`} stroke={accent} strokeWidth="1.2" opacity="0.8" />
      <circle ref={lens2Ref} cx="24" cy="28" r="3.5" fill={accent} opacity="0.9" />
      <line ref={shutterRef} x1="24" y1="22" x2="24" y2="34" stroke="white" strokeWidth="1.5" opacity="0.6" style={{ transformOrigin: '24px 28px' }} />
      {[0,1,2,3].map(i => <circle key={i} ref={el => { sparkRef.current[i] = el }} cx="24" cy="12" r={1.2 - i * 0.15} fill={accent} opacity="0.6" />)}
      <g ref={polaroid1Ref} opacity="0">
        <rect x="4" y="24" width="14" height="16" rx="2" fill="white" stroke={`${accent}60`} strokeWidth="1" />
      ...
      <g ref={polaroid2Ref} opacity="0">
        <rect x="34" y="24" width="14" height="16" rx="2" fill="white" stroke={`${accent}60`} strokeWidth="1" />
...
const ReviewsSVG = ({ accent }) => {
  ...
  const STARS = [{ cx:26,cy:14,r:5.5 },{ cx:14,cy:24,r:3.5 },{ cx:38,cy:24,r:3.5 },{ cx:18,cy:36,r:3 },{ cx:34,cy:36,r:3 }]
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      starRefs.current.forEach((el, i) => {
        if (!el) return
        gsap.to(el, { scale: 1.12, duration: 1.4 + i * 0.2, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 1.2 + i * 0.15, transformOrigin: 'center' })
      })
      if (glowRef.current) gsap.to(glowRef.current, { scale: 1.6, opacity: 0.12, duration: 1.8, ease: 'sine.inOut', yoyo: true, repeat: -1, transformOrigin: 'center' })

      const animQuote1 = () => {
        if (!quote1Ref.current) return
        gsap.set(quote1Ref.current, { x: 0, y: 4, opacity: 0, scale: 0.6 })
        gsap.timeline().to(quote1Ref.current, { opacity: 1, scale: 1, y: -4, duration: 0.4, ease: 'back.out(2.5)' }).to(quote1Ref.current, { y: -8, duration: 1.6, ease: 'sine.inOut', yoyo: true, repeat: 1 }).to(quote1Ref.current, { opacity: 0, scale: 0.7, y: -12, duration: 0.3, ease: 'power2.in' })
      }
      callsRef.current.push(gsap.delayedCall(0.3, animQuote1), gsap.delayedCall(4.2, animQuote1))

      const animQuote2 = () => {
        if (!quote2Ref.current) return
        gsap.set(quote2Ref.current, { x: 0, y: 4, opacity: 0, scale: 0.6 })
        gsap.timeline().to(quote2Ref.current, { opacity: 1, scale: 1, y: -4, duration: 0.4, ease: 'back.out(2.5)' }).to(quote2Ref.current, { y: -8, duration: 1.6, ease: 'sine.inOut', yoyo: true, repeat: 1 }).to(quote2Ref.current, { opacity: 0, scale: 0.7, y: -12, duration: 0.3, ease: 'power2.in' })
      }
      callsRef.current.push(gsap.delayedCall(1.8, animQuote2), gsap.delayedCall(5.6, animQuote2))

      if (orbitRef.current) gsap.to(orbitRef.current, { rotation: 360, duration: 8, ease: 'none', repeat: -1, transformOrigin: '26px 26px' })
    })
    return () => { callsRef.current.forEach(c => c?.kill()); callsRef.current = []; ctx.revert() }
  }, [])

  return (
    <svg width="52" height="52" viewBox="0 0 52 52" fill="none" style={{ flexShrink: 0 }}>
      <circle ref={orbitRef} cx="26" cy="26" r="22" stroke={accent} strokeWidth="0.6" strokeDasharray="3 6" opacity="0.25" />
      <circle ref={glowRef} cx="26" cy="14" r="9" fill={accent} opacity="0.08" />
      {LINES.map(([a, b], i) => <line key={i} ref={el => { lineRefs.current[i] = el }} x1={STARS[a].cx} y1={STARS[a].cy} x2={STARS[b].cx} y2={STARS[b].cy} stroke={accent} strokeWidth="0.8" opacity="0" />)}
      {STARS.map((s, i) => (
        <g key={i} ref={el => { starRefs.current[i] = el }} style={{ transformOrigin: `${s.cx}px ${s.cy}px` }}>
      ...
      <g ref={quote1Ref} opacity="0" style={{ transformOrigin: '10px 40px' }}>
        <rect x="2" y="34" width="18" height="12" rx="4" fill={`${accent}22`} stroke={accent} strokeWidth="1" />
      ...
      <g ref={quote2Ref} opacity="0" style={{ transformOrigin: '42px 40px' }}>
        <rect x="32" y="34" width="18" height="12" rx="4" fill={`${accent}22`} stroke={accent} strokeWidth="1" />
...
// ─── ExploreToastCard ─────────────────────────────────────────────────────────
const ExploreToastCard = ({ id, title, message, accent, glow, border, isDark, svg, onDismiss, onNavigate, duration = 9000 }) => {
  ...
  useEffect(() => {
    ...
    if (shimRef.current) gsap.fromTo(shimRef.current, { x: '-110%' }, { x: '110%', duration: 1.0, ease: 'power2.inOut', delay: 0.3 })
  }, [])

  useEffect(() => {
    if (!barRef.current) return
    gsap.set(barRef.current, { scaleX: 1, transformOrigin: 'left center' })
    barTweenRef.current = gsap.to(barRef.current, { scaleX: 0, duration: duration / 1000, ease: 'none', onComplete: handleDismiss })
  ...
  const handleDismiss = useCallback(() => {
    ...
    gsap.timeline().to(wrapRef.current, { y: -80, opacity: 0, scale: 0.88, duration: 0.3, ease: 'power3.in' }).call(() => onDismiss(id))
  }, [id, onDismiss])

  const pauseAll  = () => { clearTimeout(timerRef.current); barTweenRef.current?.pause() }
  const resumeAll = () => { barTweenRef.current?.resume(); timerRef.current = setTimeout(handleDismiss, 2000) }
  ...
  return (
    <div ref={wrapRef} onClick={handleClick} onMouseEnter={pauseAll} onMouseLeave={resumeAll} onTouchStart={pauseAll} onTouchEnd={resumeAll}
      style={{
        position: 'relative', overflow: 'hidden', borderRadius: 22,
        cursor: 'pointer', userSelect: 'none',
        // ✅ var(--modal-bg) — was hardcoded rgba dark/light
      ...
      <div ref={shimRef} aria-hidden style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '45%', background: isDark ? 'linear-gradient(105deg,transparent,rgba(255,255,255,0.07) 50%,transparent)' : 'linear-gradient(105deg,transparent,rgba(255,255,255,0.35) 50%,transparent)', pointerEvents: 'none', zIndex: 1, transform: 'translateX(-110%)' }} />
      <div aria-hidden style={{ position: 'absolute', top: 0, left: '6%', right: '6%', height: 1, pointerEvents: 'none', background: isDark ? 'linear-gradient(90deg,transparent,rgba(255,255,255,0.18) 50%,transparent)' : 'linear-gradient(90deg,transparent,rgba(255,255,255,1) 50%,transparent)' }} />
      <div aria-hidden style={{ position: 'absolute', left: 0, top: '12%', bottom: '12%', width: 3, borderRadius: '0 3px 3px 0', background: `linear-gradient(180deg,${accent}66,${accent},${accent}66)` }} />
      <div aria-hidden style={{ position: 'absolute', top: -20, right: 16, width: 80, height: 80, borderRadius: '50%', background: glow, filter: 'blur(20px)', opacity: 0.7, pointerEvents: 'none' }} />
      <div style={{ position: 'relative', zIndex: 2, padding: '14px 42px 14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ width: 52, height: 52, flexShrink: 0, borderRadius: 14, background: isDark ? `${accent}14` : `${accent}0e`, border: `1px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 4px 16px ${glow}`, overflow: 'hidden' }}>
          {svg}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{
            ...
            // ✅ var(--text-primary) — was hardcoded '#FFF8EE' / '#2D1000'
            color: 'var(--text-primary)',
            // ✅ FONTS.brand — was hardcoded "'Baloo 2', system-ui, sans-serif"
            fontFamily: FONTS.brand,
          }}>{title}</p>
          <p style={{
            margin: 0, fontSize: 11.5, lineHeight: 1.5, fontWeight: 440,
            // ✅ var(--text-secondary) — was hardcoded rgba
            color: 'var(--text-secondary)',
            fontFamily: FONTS.brand,
          }}>{message}</p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 7, padding: '4px 10px', borderRadius: 8, background: `${accent}18`, border: `1px solid ${accent}35` }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: accent, fontFamily: FONTS.brand, letterSpacing: '0.02em' }}>
              Tap to explore →
            </span>
          </div>
        </div>
      </div>
      <button onClick={e => { e.stopPropagation(); handleDismiss() }} style={{ position: 'absolute', top: 10, right: 10, zIndex: 3, width: 22, height: 22, borderRadius: '50%', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)', color: isDark ? 'rgba(255,220,160,0.5)' : 'rgba(80,40,10,0.4)', cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
      ...
      <div aria-hidden style={{ position: 'absolute', bottom: 0, left: '18%', right: '18%', height: 1, pointerEvents: 'none', background: `linear-gradient(90deg,transparent,${accent}80 50%,transparent)`, opacity: 0.7 }} />
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2.5 }}>
        <div ref={barRef} style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, background: `linear-gradient(90deg,${accent}aa,${accent})`, borderRadius: 2, transformOrigin: 'left center' }} />
      </div>
    </div>
  )
}
...
export const ExploreToastPortal = ({ toasts, onDismiss, onNavigate }) => {
  ...
  return createPortal(
    <div style={{ position: 'fixed', top: 'calc(env(safe-area-inset-top, 0px) + 88px)', left: 0, right: 0, zIndex: 9400, padding: '0 16px', pointerEvents: 'none' }}>
      <div style={{ position: 'relative', margin: '0 auto', width: '100%', maxWidth: 440, pointerEvents: 'auto' }}>
        {toasts.map((t, i) => (
          <div key={t.id} style={{ marginBottom: i < toasts.length - 1 ? 10 : 0 }}>
            <ExploreToastCard {...t} isDark={isDark} onDismiss={onDismiss} onNavigate={() => onNavigate(t.route)} />
          </div>
        ))}
      </div>
    </div>,
...
export const useExploreToasts = ({ weather = null, orderHistory = [], activeOrder = null, visitCount = 0, favouriteItem = null, favouriteCategory = null, tier = 'none', timeSlot = 'morning' } = {}) => {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(timer2Ref.current); clearTimeout(timer3Ref.current) }
  }, [addToast])
